...
export default function MonthlyFinanceReport({ data }) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-12 text-gray-500">
        No monthly data available for this report.
      </div>
    );
  }
  ...
  const totalNet = data.reduce((sum, row) => sum + row.netBalance, 0);
  const totalTransactions = data.reduce((sum, row) => sum + row.transactionCount, 0);
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b-2 border-gray-100">
            <th className="py-4 px-4 text-sm font-bold text-gray-400 uppercase tracking-wider">Month</th>
            <th className="py-4 px-4 text-sm font-bold text-gray-400 uppercase tracking-wider text-right">Transactions</th>
            ...
            <th className="py-4 px-4 text-sm font-bold text-gray-400 uppercase tracking-wider text-right">Net Balance</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-50">
          {data.map((row, index) => (
            <tr key={index} className="hover:bg-gray-50 transition-colors">
              ...
              <td className={`py-4 px-4 font-bold text-right ${row.netBalance >= 0 ? 'text-church-navy' : 'text-amber-500'}`}>
        ...
        <tfoot>
          <tr className="border-t-2 border-gray-100 bg-gray-50/50">
            ...
            <td className="py-4 px-4 font-extrabold text-gray-500 text-right">{totalTransactions}</td>
